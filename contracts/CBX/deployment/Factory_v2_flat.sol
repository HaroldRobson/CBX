pragma solidity <0.9.0 >=0.4.16 >=0.4.22 >=0.5.0 >=0.6.2 >=0.8.4 ^0.8.0 ^0.8.20 ^0.8.24 ^0.8.30;
...
contract Factory {
    ...
    */
    ...
    function deActivatePool(bool shouldRefund, uint256 refundAmount) external {
        // this can only be successfully called by an existing pool only becasue of msg.sender
        ...
        emit poolDeactivated(
            msg.sender,
            sellerAddress,
            addressToPool[msg.sender].IPFS_URI,
            shouldRefund,
            addressToPool[msg.sender].registry
        );
        if (shouldRefund) {
            emit refundSeller(
                msg.sender,
