...
//     }
//
//     function sendReceipts(bytes memory retirementBundle, address originalPool) external onlyOwner {
//         PendingRetirement[] memory retirements = abi.decode(retirementBundle, (PendingRetirement[]));
//         for (uint256 i = 0; i < retirements.length; i++) {
//             mintWithData(retirements[i].user, originalPool, retirements[i].tokens, retirements[i].timestamp);
//         }
//     }
